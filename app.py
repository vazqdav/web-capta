...
def historial():
    ...
    page = int(request.args.get('page', 1))  # Obtener el número de página actual
    limit = 7  # Número de registros por página
    skip = (page - 1) * limit  # Número de registros a omitir
    ...
    total_registros = registros_collection.count_documents(query)
    registros = list(registros_collection.find(query).skip(skip).limit(limit))
...
@app.route('/registroshistorial/<mesAno>', methods=['GET'])
def registroshistorial(mesAno):
    año, mes = mesAno.split('-')
    query = {'fecha': {'$regex': f'^{año}-{mes.zfill(2)}'}}
    registros = list(registros_collection.find(query))
    return render_template('Registroshistorial.html', registros=registros, mes=mes, año=año)
...
def ver_registros():
    ...
    return render_template('Registroadmin.html', registros=registros, page=page, total_pages=total_pages)

@app.route('/busca_usuario', methods=['POST'])
def busca_usuario():
    search_query = request.json.get('search_query', '')
    if search_query:
        registros = list(registros_collection.find({"nombre_paciente": {"$regex": search_query, "$options": "i"}}))
...
# Esta función debería manejar la subida y entrega de archivos
@app.route('/archivo_subidos/<filename>')
def archivos_subidos(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)


@app.route('/ver_masadmin/<registro_id>')
def ver_masadmin(registro_id):
    registro = registros_collection.find_one({'_id': ObjectId(registro_id)})
    if not registro:
        return redirect(url_for('mostrar_registros'))

    return render_template('Vermasadmin.html', registro=registro)
